...
class _VolumePlusScreenState extends State<VolumePlusScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (settings.volumePlusTableScrollPosition != null && tableScrollController.hasClients) {
        tableScrollController.jumpTo(settings.volumePlusTableScrollPosition!);
      }
    });
    ...
    tableScrollController.addListener(() {
      if (tableScrollController.hasClients) {
        final settings = context.read<Settings>();
        settings.volumePlusTableScrollPosition = tableScrollController.offset;
      }
    });

    updateModelOptions(settings.volumePlusInAdultView);
    run(initState: true);
  }

  void _setControllersFromSettings(Settings settings) {
    tableController.val = settings.isVolumePlusTableExpanded;

    if (settings.volumePlusInAdultView) {
      adultModelController.selection = settings.volumePlusAdultModel;
      sexController.val = settings.volumePlusAdultSex == Sex.Female ? true : false;
      ageController.text = settings.volumePlusAdultAge?.toString() ?? '';
      heightController.text = settings.volumePlusAdultHeight?.toString() ?? '';
      weightController.text = settings.volumePlusAdultWeight?.toString() ?? '';
      targetController.text = settings.volumePlusPropofolTarget?.toStringAsFixed(1) ?? '';
      durationController.text = settings.volumePlusAdultDuration?.toString() ?? '';
    } else {
      pediatricModelController.selection = settings.volumePlusPediatricModel;
      sexController.val = settings.volumePlusPediatricSex == Sex.Female ? true : false;
      ageController.text = settings.volumePlusPediatricAge?.toString() ?? '';
      heightController.text = settings.volumePlusPediatricHeight?.toString() ?? '';
      weightController.text = settings.volumePlusPediatricWeight?.toString() ?? '';
      targetController.text = settings.volumePlusPediatricTarget?.toStringAsFixed(1) ?? '';
      durationController.text = settings.volumePlusPediatricDuration?.toString() ?? '';
    }
  }
  ...
  void run({initState = false}) {
    ...
    if (initState == false) {
      if (settings.volumePlusInAdultView) {
        settings.volumePlusAdultModel = adultModelController.selection;
        settings.volumePlusAdultSex = sex;
        settings.volumePlusAdultAge = age;
        settings.volumePlusAdultHeight = height;
        settings.volumePlusAdultWeight = weight;
        settings.volumePlusPropofolTarget = target;
        settings.volumePlusAdultDuration = duration;
      } else {
        settings.volumePlusPediatricModel = pediatricModelController.selection;
        settings.volumePlusPediatricSex = sex;
        settings.volumePlusPediatricAge = age;
        settings.volumePlusPediatricHeight = height;
        settings.volumePlusPediatricWeight = weight;
        settings.volumePlusPediatricTarget = target;
        settings.volumePlusPediatricDuration = duration;
      }
    }
    ...
    if (age != null &&
      ...
      if (age >= 0 &&
          ...
          target >= 0 &&
          duration >= 0) {
        Model model = age >= 17 ? settings.volumePlusAdultModel : settings.volumePlusPediatricModel;
  ...
  void updatePDTableController(PDTableController controller) {
    final settings = Provider.of<Settings>(context, listen: false);
    settings.isVolumePlusTableExpanded = !settings.isVolumePlusTableExpanded;

    setState(() {
      _shouldAnimateTableExpansion = true; // Enable animation for button tap
      controller.val = settings.isVolumePlusTableExpanded;
    });
  ...
  void reset({bool toDefault = false}) {
    ...
    if (settings.volumePlusInAdultView) {
      sexController.val = toDefault
          ? true
          : settings.volumePlusAdultSex == Sex.Female
              ? true
              : false;
      ageController.text = toDefault
          ? 40.toString()
          : settings.volumePlusAdultAge != null
              ? settings.volumePlusAdultAge.toString()
              : '';
      heightController.text = toDefault
          ? 170.toString()
          : settings.volumePlusAdultHeight != null
              ? settings.volumePlusAdultHeight.toString()
              : '';
      weightController.text = toDefault
          ? 70.toString()
          : settings.volumePlusAdultWeight != null
              ? settings.volumePlusAdultWeight.toString()
              : '';
      targetController.text = toDefault
          ? 3.0.toStringAsFixed(1)
          : settings.volumePlusPropofolTarget != null
              ? settings.volumePlusPropofolTarget!.toStringAsFixed(1)
              : '';
      durationController.text = toDefault
          ? 60.toString()
          : settings.volumePlusAdultDuration != null
              ? settings.volumePlusAdultDuration.toString()
              : '';
    } else {
      sexController.val = toDefault
          ? true
          : settings.volumePlusPediatricSex == Sex.Female
              ? true
              : false;
      ageController.text = toDefault
          ? 8.toString()
          : settings.volumePlusPediatricAge != null
              ? settings.volumePlusPediatricAge.toString()
              : '';
      heightController.text = toDefault
          ? 130.toString()
          : settings.volumePlusPediatricHeight != null
              ? settings.volumePlusPediatricHeight.toString()
              : '';
      weightController.text = toDefault
          ? 26.toString()
          : settings.volumePlusPediatricWeight != null
              ? settings.volumePlusPediatricWeight.toString()
              : '';
      targetController.text = toDefault
          ? 3.0.toStringAsFixed(1)
          : settings.volumePlusPediatricTarget != null
              ? settings.volumePlusPediatricTarget!.toStringAsFixed(1)
              : '';
      durationController.text = toDefault
          ? 60.toString()
          : settings.volumePlusPediatricDuration != null
              ? settings.volumePlusPediatricDuration.toString()
              : '';
    }
  ...
  void updatePDTextEditingController() {
    ...
    int? age = int.tryParse(ageController.text);
    if (age != null) {
      settings.volumePlusInAdultView = age >= 17 ? true : false;
    }
    updateModelOptions(settings.volumePlusInAdultView);
    run();
  }
  ...
  Widget buildModelSelector(Settings settings, double UIHeight) {
    final currentModel = settings.volumePlusInAdultView
    ...
    final hasValidationError = currentModel != null &&
        (settings.volumePlusInAdultView ? adultModelController : pediatricModelController)
          ...
          Positioned.fill(
            child: GestureDetector(
              onTap: () async {
                ...
                final controller = settings.volumePlusInAdultView ? adultModelController : pediatricModelController;
                controller.showModelSelector(
                  context: context,
                  inAdultView: settings.volumePlusInAdultView,
                  ...
                  onModelSelected: (model) {
                    setState(() {
                      if (settings.volumePlusInAdultView) {
                        ...
                        settings.volumePlusAdultModel = model;
                      } else {
                        pediatricModelController.selection = model;
                        settings.volumePlusPediatricModel = model;
                      }
                    });
  ...
  Widget build(BuildContext context) {
    ...
    Model selectedModel = settings.volumePlusInAdultView
    ...
    final bool heightTextFieldEnabled = settings.volumePlusInAdultView
    ...
    final bool sexSwitchControlEnabled = settings.volumePlusInAdultView
        ? (adultModelController.selection as Model).target != Target.Plasma
        : (pediatricModelController.selection as Model).target != Target.Plasma;

    final ageTextFieldEnabled = !(settings.volumePlusInAdultView &&
                    ...
                    children: [
                      GestureDetector(
                        onTap: () async {
                          ...
                          if (settings.volumePlusInAdultView) {
                            ageController.text = settings.volumePlusPediatricAge != null
                                ? settings.volumePlusPediatricAge.toString()
                                : '';
                          } else {
                            ageController.text = settings.volumePlusAdultAge != null
                                ? settings.volumePlusAdultAge.toString()
                                : '';
                          }
                          settings.volumePlusInAdultView = !settings.volumePlusInAdultView;
                          reset();
                        },
                        child: Chip(
                          avatar: settings.volumePlusInAdultView
                              ? Icon(
                                  Icons.face,
                          ...
                          label: Text(
                            settings.volumePlusInAdultView ? AppLocalizations.of(context)!.adult : AppLocalizations.of(context)!.paed,
                            style: TextStyle(
                                color: Theme.of(context).colorScheme.onPrimary),
                          ),
                      ...
                      ? Consumer<Settings>(
                          builder: (context, settings, child) {
                            return AnimatedDataTable(
                              ...
                              selectedRowIndex: settings.selectedVolumePlusTableRow,
                               onRowTap: (index) {
                                 // Toggle selection: if same row is tapped, deselect it
                                 if (settings.selectedVolumePlusTableRow == index) {
                                   settings.selectedVolumePlusTableRow = null;
                                 } else {
                                   settings.selectedVolumePlusTableRow = index;
                                 }
                                },
                      ...
                      children: [
                        Expanded(
                          child: PDSwitchField(
                            ...
                            settings.volumePlusInAdultView ? Icons.woman : Icons.girl :
                            settings.volumePlusInAdultView ? Icons.man : Icons.boy,
                            controller: sexController,
                            switchTexts: {
                              true: settings.volumePlusInAdultView ? Sex.Female.toLocalizedString(context): Sex.Girl.toLocalizedString(context),
                              false: settings.volumePlusInAdultView ? Sex.Male.toLocalizedString(context): Sex.Boy.toLocalizedString(context)
                            },
                            onChanged: run,
                            height: UIHeight,
...
class VolumePlusScreen extends StatefulWidget {
  const VolumePlusScreen({super.key});

  @override
  State<VolumePlusScreen> createState() => _VolumePlusScreenState();
}
